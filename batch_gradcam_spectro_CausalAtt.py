...
# batch_gradcam_spectro_CausalAtt.py
# GradCAM for CausalAtt model
...
def run_gradcam_for_spectrograms():
    root_dir = "Voice"
    output_dir = "gradcam_outputs_CausalAtt"
    model_path = "saved_models/CausalAtt_seed0.pth"
    os.makedirs(output_dir, exist_ok=True)

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = CausalSpectroNet(use_causal=False).to(device)
